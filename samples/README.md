...
| Modular KMP | [`modular-kmp-data-layer`](modular-kmp-data-layer/) | Feature two uses shared data while Feature One is native and visible. | Feature One lives in platform `featureone` / `Sources/FeatureOne` code and renders FeatureOneScreen/View -> HomeViewModel -> TaskRepository -> TaskDataSource. |
| Modular KMP | [`modular-kmp-presentation-layer`](modular-kmp-presentation-layer/) | Feature two has shared Compose presentation while Feature One is native and visible. | Feature One lives in platform `featureone` / `Sources/FeatureOne` code and renders FeatureOneScreen/View -> HomeViewModel -> TaskRepository -> TaskDataSource. |
| Modular KMP | [`modular-kmp-ui-layer`](modular-kmp-ui-layer/) | Feature two shares Compose UI while Feature One is native and visible. | Feature One lives in platform `featureone` / `Sources/FeatureOne` code and renders FeatureOneScreen/View -> HomeViewModel -> TaskRepository -> TaskDataSource. |
| Modular KMP | [`modular-kmp-ui-data-layer`](modular-kmp-ui-data-layer/) | Feature two shares Compose UI and data while Feature One is native and visible. | Feature One lives in platform `featureone` / `Sources/FeatureOne` code and renders FeatureOneScreen/View -> HomeViewModel -> TaskRepository -> TaskDataSource. |
