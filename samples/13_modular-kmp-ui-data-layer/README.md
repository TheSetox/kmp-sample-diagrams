...
```sh
./gradlew -p samples/13_modular-kmp-ui-data-layer :desktopApp:compileKotlin
./gradlew -p samples/13_modular-kmp-ui-data-layer :androidApp:assembleDebug
./gradlew -p samples/13_modular-kmp-ui-data-layer :featureTwoSharedData:assemble
./gradlew -p samples/13_modular-kmp-ui-data-layer :featureTwoSharedUI:compileKotlinDesktop
```
...
Open `iosApp/iosApp.xcodeproj` and run the shared `iosApp` scheme. Its `Build KMP Frameworks` phase builds and copies both required frameworks automatically.
